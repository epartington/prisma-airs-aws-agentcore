"""
Tutorial 00 — Getting Started + Prisma AIRS (MODIFIED)
────────────────────────────────────────────────────────
HOOK MAP — what changed from main_original.py:

  Line  Change
  ────  ──────────────────────────────────────────────────────────────────
   18   Added: from security_layer import airs_protected_tool, AIRSBlockedError
   19   Added: from security_layer import scan_prompt
   70   Added: @airs_protected_tool above @tool on get_return_policy
         → PreToolUse + PostToolUse scanning on every return policy lookup
   83   Added: @airs_protected_tool above @tool on get_product_info
         → PreToolUse + PostToolUse scanning on every product search
  121   Added: UserPromptSubmit AIRS scan block before agent is called
         → blocks malicious prompts before the LLM ever sees them

That's it — 4 insertion points.  The rest of the file is unchanged.
Run `diff main_original.py main_with_airs.py` to see the exact diff.
"""

import logging
import os

from strands import Agent, tool
from bedrock_agentcore.runtime import BedrockAgentCoreApp
from model.load import load_model

# ── CHANGE 1: import the AIRS security layer ─────────────────────────────────
# This is the only new import.  security_layer/ lives next to this tutorials/
# directory — adjust sys.path if you move files.
from security_layer import airs_protected_tool, AIRSBlockedError
from security_layer import scan_prompt

logging.basicConfig(level=logging.INFO)
logger = logging.getLogger(__name__)

app = BedrockAgentCoreApp()
log = app.logger

RETURN_POLICIES = {
    "electronics": {
        "window": "30 days",
        "condition": "Original packaging required, must be unused or defective",
        "refund": "Full refund to original payment method",
    },
    "accessories": {
        "window": "14 days",
        "condition": "Must be in original packaging, unused",
        "refund": "Store credit or exchange",
    },
    "audio": {
        "window": "30 days",
        "condition": "Defective items only after 15 days",
        "refund": "Full refund within 15 days, replacement after",
    },
}

PRODUCTS = {
    "PROD-001": {"name": "Wireless Headphones", "price": 79.99, "category": "audio",
                 "description": "Noise-cancelling Bluetooth headphones with 30h battery life", "warranty_months": 12},
    "PROD-002": {"name": "Smart Watch", "price": 249.99, "category": "electronics",
                 "description": "Fitness tracker with heart rate monitor, GPS, and 5-day battery", "warranty_months": 24},
    "PROD-003": {"name": "Laptop Stand", "price": 39.99, "category": "accessories",
                 "description": "Adjustable aluminum laptop stand for ergonomic desk setup", "warranty_months": 6},
    "PROD-004": {"name": "USB-C Hub", "price": 54.99, "category": "accessories",
                 "description": "7-in-1 USB-C hub with HDMI, USB-A, SD card reader, and ethernet", "warranty_months": 12},
    "PROD-005": {"name": "Mechanical Keyboard", "price": 129.99, "category": "electronics",
                 "description": "RGB mechanical keyboard with Cherry MX switches", "warranty_months": 24},
}


# ── CHANGE 2: add @airs_protected_tool to get_return_policy ──────────────────
# The @airs_protected_tool decorator must be ABOVE @tool.
# Decorator execution order in Python is bottom-up:
#   1. @tool registers the function with Strands
#   2. @airs_protected_tool wraps the already-registered function
# Result: every call to get_return_policy triggers an AIRS pre+post scan.
@airs_protected_tool    # ← ADDED (PreToolUse + PostToolUse scanning)
@tool
def get_return_policy(product_category: str) -> str:
    """Get return policy information for a specific product category.

    Args:
        product_category: Product category (e.g., 'electronics', 'accessories', 'audio')

    Returns:
        Formatted return policy details including timeframes and conditions
    """
    category = product_category.lower()
    if category in RETURN_POLICIES:
        policy = RETURN_POLICIES[category]
        return (f"Return policy for {category}: Window: {policy['window']}, "
                f"Condition: {policy['condition']}, Refund: {policy['refund']}")
    return f"No specific return policy found for '{product_category}'. Please contact support."


# ── CHANGE 3: add @airs_protected_tool to get_product_info ───────────────────
@airs_protected_tool    # ← ADDED
@tool
def get_product_info(query: str) -> str:
    """Search for product information by name, ID, or keyword.

    Args:
        query: Product name, ID (e.g., 'PROD-001'), or search keyword

    Returns:
        Product details including name, price, category, and description
    """
    query_lower = query.lower()
    if query.upper() in PRODUCTS:
        p = PRODUCTS[query.upper()]
        return (f"{p['name']} ({query.upper()}): ${p['price']}, Category: {p['category']}, "
                f"{p['description']}, Warranty: {p['warranty_months']} months")
    results = [
        f"{pid}: {p['name']} - ${p['price']} - {p['description']}"
        for pid, p in PRODUCTS.items()
        if query_lower in p['name'].lower() or query_lower in p['description'].lower()
           or query_lower in p['category'].lower()
    ]
    if results:
        return "Found products:\n" + "\n".join(results)
    return f"No products found matching '{query}'."


SYSTEM_PROMPT = """You are a helpful and professional customer support assistant.

All tool calls are inspected by Prisma AIRS for security compliance.
Always use the appropriate tool rather than guessing."""

_agent = None


def get_or_create_agent():
    global _agent
    if _agent is None:
        _agent = Agent(
            model=load_model(),
            system_prompt=SYSTEM_PROMPT,
            tools=[get_return_policy, get_product_info],
        )
    return _agent


@app.entrypoint
async def invoke(payload, context):
    log.info("Invoking AIRS-protected agent...")
    user_prompt = payload.get("prompt", "")

    # ── CHANGE 4: UserPromptSubmit AIRS scan ──────────────────────────────────
    # This mirrors the Claude Code UserPromptSubmit shell hook but runs in-process.
    # The scan happens before the LLM ever sees the message — malicious prompts
    # are blocked here without consuming any LLM tokens.
    try:
        pre = scan_prompt(prompt=user_prompt, source="user-prompt-submit")
        if pre.blocked:
            yield f"[AIRS BLOCKED] Your request was blocked: {pre.category}"
            return
        log.info("AIRS UserPromptSubmit: action=%s detections=%s", pre.action, pre.detections)
    except Exception as exc:
        log.error("AIRS UserPromptSubmit scan failed: %s — blocking (fail-closed)", exc)
        yield "[AIRS ERROR] Security scan failed. Request blocked."
        return

    agent = get_or_create_agent()
    try:
        stream = agent.stream_async(user_prompt)
        async for event in stream:
            if "data" in event and isinstance(event["data"], str):
                yield event["data"]
    except AIRSBlockedError as exc:
        yield f"[AIRS BLOCKED] {exc}"


if __name__ == "__main__":
    app.run()
